 ...
 *
 * Display behaviour:
 * - Short explanation (~150 chars) is visible initially.
 * - Full explanation is hidden inside an accessible accordion.
 *
...
function _buildResultCard(exp, rec, idx) {
  ...
  const score   = typeof exp.semantic_score === 'number'
    ? exp.semantic_score.toFixed(4)
    : '—';
        ...
        ? `
          ...
          >
            ...
            <div class="px-5 py-4">
              ...
              <p
                class="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-2.5 flex items-center gap-1.5"
              >

                <svg
                  class="w-3.5 h-3.5 text-govorange"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
              ...
              <div
                class="explanation-text text-slate-600 text-xs leading-relaxed whitespace-pre-wrap"
              >
                ${_formatExplanation(fullExplanation)}
              </div>
...
function toggleCard(bodyId) {
  ...
  const idx =
    bodyId.replace('result-body-', '');

  const chevron =
    document.getElementById(`chevron-${idx}`);

  const label =
    document.getElementById(`toggle-label-${idx}`);

  const btn =
    document.querySelector(
      `button[aria-controls="${bodyId}"]`
    );
...
function _resetResults() {
  document
    .getElementById('extraction-summary')
    ?.classList.add('hidden');
